...
Two design decisions underpin auditability. Commands such as `build`, `dump`, and `update` write a sidecar manifest by default, recording software versions, the command line, input files and their hashes, source sheets where applicable, and output information. This record allows a control file to be traced to its source workbook. Round-tripping is assessed by the meaning of supported content rather than by identical text: rewriting a file may change spacing, write identifiers in lowercase, or represent the same numeric value as `1.0` rather than `1`. The `diff` command compares supported settings and table values with numeric tolerances. Semantic agreement therefore need not imply byte-for-byte identity.
...
Input tables use the column structure summarized in Table 1, with headers in the first row. Recognized headers are case-insensitive. Data-sheet names are chosen by the user and mapped to table roles in the command; for example, `--add_par_xls calibration.xlsx,PAR_HK` selects the `PAR_HK` sheet as a parameter table. Repeating the option combines multiple parameter sheets. The workbook shortcut illustrated above instead reads these mappings from a sheet named `BUILD`. Parameter, observation, and group identifiers must correspond across the tables and model interface files. Additional helper columns, such as source references, indices, or notes, remain in the workbook but are excluded from the control file when they are not recognized input fields.
...
A workflow can also use CSV inputs without a workbook. Options such as `--add_par_csv par.csv` and `--add_obs_csv obs.csv` select the same table roles; repeated options combine tables supplied in multiple files. One CSV can consolidate data from several worksheets, and helper-only sheets need no CSV counterpart. CSV inputs contain values rather than workbook formulas or features. The [four-parameter CSV tutorial](https://github.com/ougx/makePst/blob/v0.3.0/README.md#two-minute-tutorial) provides a complete example. These source CSV tables are distinct from the external tables that makePst can write as part of a PEST++ version-2 output.
...
**`dump`** reads supported control-file content into a new workbook whose sheets `build` can read back. This brings an inherited control file into the spreadsheet workflow. It does not recover source formulas, annotations, or workbook layout that were not stored in the control file.

**`update`** returns parameter, residual, or PESTPP-IES ensemble results to an existing workbook. It locates parameter and observation rows by `PARNME` and `OBSNME` across worksheets and writes the requested result fields. Before writing those table cells, it identifies cells containing formulas and skips them by default; the user can explicitly request replacement with `--overwrite_formulas`. For example, residual results can populate `MODELLED` and `RESIDUAL` while a formula in `WEIGHT` remains intact. Residual updates also report objective-function contributions by observation group.

Formula preservation and formula recalculation are separate operations. The openpyxl backend updates workbook files without installed Excel but does not recalculate formulas. The xlwings backend operates through installed desktop Excel and requests recalculation when saving. Open-source xlwings supports Windows and macOS ([xlwings installation documentation](https://docs.xlwings.org/en/stable/installation.html)); the native-Excel demonstration reported here used Windows. The tests reported here do not establish makePst's xlwings support on macOS. The scope section describes limitations on workbook features and cached values.
